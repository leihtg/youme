 ...
 */
public class CircleImageViewCustom extends ImageView {
    ...
    public CircleImageViewCustom(Context context, AttributeSet attrs) {
        super(context, attrs);
        //取xml文件中设定的参数
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CircleImageViewCustom);
        mType = ta.getInt(R.styleable.CircleImageViewCustom_type, DEFAULT_TYPE);
        mBorderColor = ta.getColor(R.styleable.CircleImageViewCustom_borderColor, DEFAULT_BORDER_COLOR);
        mBorderWidth = ta.getDimensionPixelSize(R.styleable.CircleImageViewCustom_borderWidth, dip2px(DEFAULT_BORDER_WIDTH));
        mRectRoundRadius = ta.getDimensionPixelSize(R.styleable.CircleImageViewCustom_rectRoundRadius, dip2px(DEFAULT_RECT_ROUND_RADIUS));
        ta.recycle();
    }
